...
class TrainingText(BaseModel):
    ...
    Attributes:
        ...
        group_id (str | None): ID of the group. It is used by the RL finetuning script to normalize rewards.
...
class Step(BaseModel):
    ...
    Methods:
        ...
        llm_view(indent: int | None = 2) -> str:
...
class TapeMetadata(BaseModel):
    ...
    Attributes:
        ...
        parent_id (str | None): An optional identifier for the parent tape.
        author (str | None): An optional name of the author of the tape.
        author_tape_id (str | None): An optional identifier for the author's tape.
        n_added_steps (int): The number of steps added to the tape in the last agent run, default is 0.
        error (Any | None): An optional field to store any errors occured during the last agent run.
        result (Any): Optional field to store the result associated with the tape, default is an empty dictionary.
    """
...
class Tape(BaseModel, Generic[ContextType, StepType]):
    ...
    Attributes:
        ...
        context (ContextType | None): Context information for the tape.
        steps (List[StepType]): List of steps in the tape.

    Methods:
        __iter__() -> Iterator[StepType]:
            Returns an iterator over the steps in the tape.

        __len__() -> int:
            Returns the number of steps in the tape.

        __getitem__(key: int | slice) -> StepType | Self:
            Retrieves a step or a slice of steps from the tape.

        __add__(tape: Self | Iterable[Step]) -> Self:
            Concatenates two tapes or appends a list of steps to the tape.

        append(step: StepType) -> Self:
            Adds a step to the tape and creates new metadata.
...
class Prompt(BaseModel):
    ...
    Attributes:
        ...
        tools (list[dict] | None): A list of tools associated with the prompt, default is None.
        messages (list[dict]): A list of messages in the prompt, default is an empty list.

    Methods:
        from_user_message(content: str) -> Prompt:
            Creates a Prompt instance from a user message.

        __bool__() -> bool:
            Returns True if there are any messages in the prompt, otherwise False.
    """
...
class AgentEvent(BaseModel, Generic[TapeType]):
    ...
    Attributes:
        step (SerializeAsAny[Step] | None): A step produced by the agent.
        partial_step (PartialStep | None): A partial step produced by the agent.
        partial_tape (TapeType | None): A partial tape produced by the agent.
        final_tape (TapeType | None): A final tape produced by the agent.
    """
