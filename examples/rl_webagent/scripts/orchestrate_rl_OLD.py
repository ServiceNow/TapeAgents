...
from ..steps import WebAction, WebTask
from ..utils import (
    VLLMServiceManager,
    calculate_stats,
    clean_up,
...
def convert_webtask_to_tapes(webtasks: list[dict]) -> list[WebTape]:
    """
    Creates WebTape objects from a list of web task dictionaries.

    Args:
        webtasks (list[dict]): List of dictionaries containing web tasks, where each dict
            has 'task' and expected answer. The list is created from a dataset.

    Returns:
        list[WebTape]: List of WebTape objects initialized with the web tasks as WebTask steps.
    """
    tapes: list[WebTape] = []
    for webtask in tqdm(webtasks, desc="Converting web tasks to unique tapes", unit="task"):
        start_step = WebTask(
            task=webtask["task"],
            metadata=StepMetadata(
                other={
                    "expected_answer": webtask["answer"],
                }
            ),
        )
        tape = WebTape(steps=[start_step], context=None)
        tapes.append(tape)
    return tapes


def load_webtasks() -> Tuple[list, list]:
    """
    Load web tasks instead of GSM8K dataset.

    Returns:
        Tuple[list, list]: Train and test task lists
    """
...
def batch_run_agent_replica(
    agent: WebAgent, env: WebEnvironment, tapes: list[WebTape]
) -> tuple[WebAgent, list[WebTape]]:
    """
    Run agent on a batch of tapes with web environment interaction.

    Args:
        agent: The web agent
        env: The web environment
        tapes: List of tapes to run

    Returns:
        tuple[WebAgent, list[WebTape]]: Updated agent and completed tapes
    """
    final_tapes = []

    for tape in tapes:
        task = tape.steps[0].metadata.other["task"]
        seed = tape.steps[0].metadata.other["seed"]

        # Start task in environment
        ...
        for event in main_loop(agent, tape, env, max_loops=20):
            if event.agent_event and event.agent_event.step:
                ...
                immediate_reward = env.get_step_reward(step)
...
def generate_training_data(
    ...
    Args:
        agent_replicas: List of WebAgents that interact with the web environment
        envs: List of WebEnvironments (one per agent replica)
        tapes: List of tapes initialized with web tasks
    ...
    ### STEP 1: run the agent on the tapes in parallel ###
    ...
    with ProcessPoolExecutor(max_workers=len(agent_replicas)) as executor:
        ...
        # Pass both agent and environment to each worker
        futures = [
            executor.submit(batch_run_agent_replica, agent, env, tape_subset)
            for agent, env, tape_subset in zip(agent_replicas, envs, replica_tapes)
        ]
        ...
        try:
            ...
            with VLLMServiceManager(
                ...
                **(dict(cfg.vllm_config.vllm_kwargs) | dict(cfg.vllm_config.actor_vllm_kwargs)),
                ...
                ### Step 2.1: create train & test splits with list[WebAgent] and list[WebTape] ###
                sub_samples = random.sample(train_samples, cfg.max_agent_forks // cfg.attempts)
                train_tapes = convert_webtask_to_tapes(sub_samples)
                ...
                train_envs = [WebEnvironment(**cfg.env) for _ in range(len(train_agent_replicas))]

                # Create train split
                logger.info(
                    f"Creating train split with {len(train_agent_replicas)} agents and {len(train_tapes)} tapes"
                )
                splits = [("train", train_agent_replicas, train_envs, train_tapes)]

                # Create test split if needed
                if state["iteration"] % cfg.test_every_n_iterations == 0 and cfg.test_every_n_iterations > 0:
                    test_tapes = convert_webtask_to_tapes(test_samples)
                    test_llms = [
                        TrainableLLM(
                            base_url=base_url,
                            model_name=str(assistant_model_path),
                    ...
                    test_envs = [WebEnvironment(**cfg.env) for _ in range(len(test_agent_replicas))]
                    splits.append(("test", test_agent_replicas, test_envs, test_tapes))

                ### Step 2.2: generate continuations for each split ###
                for split_name, agent_replicas, envs, tapes in splits:
                    ...
                    agent_replicas_with_stats, new_tapes, split_training_samples, stats = generate_training_data(
                        agent_replicas, envs, tapes, tapes_dir, split_name
                    )
