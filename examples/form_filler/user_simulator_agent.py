...
from tapeagents.core import MakeObservation, Prompt, Error
...
class SampleUserInstructionThought(Thought):
    kind: Literal["sample_user_instruction_thought"] = "sample_user_instruction_thought"
    instruction_alias: str 
    instruction: str


class UserSimulatorError(Action, Error):
    kind: Literal["user_simulator_error"] = "user_simulator_error"
    error: str
...
class InstructionSampler(BaseModel):
    ...
    def from_str(cls, instruction_str: str) -> 'InstructionSampler':
        ...
        return cls(
            instructions={"default_instruction": instruction_str},
            weights={"default_instruction": 1.0}
        )
...
class UserSimulatorAgent(ObservationMaker[FormFillerTape, UserSimulatorTape]):
    ...
    def create(cls, instruction: str | InstructionSampler, **kwargs) -> 'UserSimulatorAgent':
        ...
        assert len(user_simulator_agent.step_filters) <= len(user_simulator_agent._step_types_chain), 'Step filters must be less than or equal to the step types chain'
        ...
        user_simulator_agent._step_filters = (user_simulator_agent.step_filters 
            + [AlwaysTrue() for __ in range(len(user_simulator_agent._step_types_chain) - len(user_simulator_agent.step_filters)) ])
    ...
    def make_own_tape(self, tape: FormFillerTape) -> UserSimulatorTape:
        ...
        tape_without_setnextnode = tape.model_copy(
            update={
                'steps': [step for step in tape.steps if not isinstance(step, SetNextNode)]
            }
        )
    ...
    def can_continue(self, tape: FormFillerTape) -> bool:
        ...
        if isinstance(state, FormFillerStateError):
            logger.error(f'Failed to compute state for tape_without_setnextnode {tape_without_setnextnode.metadata.id}: {state}')
            return False
        ...
        for i, (step, step_type, step_filter) in enumerate(zip(last_steps, self._step_types_chain, self._step_filters)):
            ...
            if not isinstance(step, step_type):
                logger.debug(f"Step {actual_index} of tape_without_setnextnode {tape_without_setnextnode.metadata.id} is not of type {step_type}: ")
                logger.debug(f"Expected: {target_step_classes}, got: {actual_step_classes}")
                return False
            if not step_filter.filter(state, step):
                logger.debug(f"Step {actual_index} of tape_without_setnextnode {tape_without_setnextnode.metadata.id} failed the filter {repr(step_filter)}")
                return False
...
class UserSimulatorMainNode(Node):
    ...
    def make_prompt(self, agent: UserSimulatorAgent, tape: UserSimulatorTape) -> Prompt:
        ...
        assert isinstance(tape.context, FormFillerTape), f'UserSimulatorTape must have FormFillerTape context'
        assert isinstance(tape.steps[-1], SampleUserInstructionThought), f'Last step must be SampleUserInstructionThought'
        ...
        schema_placeholders = {
            ...
            "all_schema_descriptions": json.dumps(
                dict((schema.name, schema.description) for schema in environment.available_schemas.values()),
                indent=2
            ),
            "all_schema_names": json.dumps(
                [schema.name for schema in environment.available_schemas.values()],
                indent=2
            ),
        }
        ...
        if len(agent._step_types_chain) >= 2 and agent._step_types_chain[-2] == RequestFunctionParameters:
            ...
            assert not isinstance(state, FormFillerStateError), f"State should not be an error"

            request_function_parameters_step = tape.context.steps[-2]
            assert isinstance(request_function_parameters_step, RequestFunctionParameters), f"Expected RequestFunctionParameters, got {request_function_parameters_step}"
