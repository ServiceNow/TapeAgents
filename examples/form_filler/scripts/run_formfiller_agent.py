...
def run_formfiller_agent(tape: FormFillerTape, agent: Agent[FormFillerTape]) -> tuple[FormFillerTape, FormFillerTape|Exception]:
    """
    ...
    try:
        ...
        assert isinstance(tape.metadata, FormFillerUserMetadata), f"Expected FormFillerUserMetadata as input tape, got {type(tape.metadata)}. Tape: {tape.metadata.id}"
        # create environment
        assert isinstance(tape.context, FormFillerContext), f"Expected FormFillerContext, got {type(tape.context)}. Tape: {tape.metadata.id}"
